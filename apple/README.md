...
鱼饼是一个只面向 Apple 平台的 SwiftUI 文件与媒体资料库，包含 iPhone、iPad、Mac 和 Apple Watch 四个应用目标。
...
- 影音：本地队列、进度控制、上一首/下一首、后台播放、系统正在播放控制和视频播放。
- 图库：从照片或文件导入、缩略图、全屏查看与缩放。
- iPhone 到 Watch：复杂电子书会先在手机端转换为带章节、封面和阅读位置的鱼饼离线书籍包，再由 WatchConnectivity 后台传输；PDF、图片、影音保持原文件传输。
- Apple Watch：只管理鱼饼的私有资料库，可建文件夹、重命名、移动、收藏和删除；可离线选章阅读、自动翻页、管理书签、查看封面和记录时长，也可独立播放已经收到的影音。
...
- 影音：`.mp3`、`.m4a`、`.aac`、`.wav`、`.caf`、`.flac`、`.alac`、`.mp4`、`.m4v`、`.mov` 等 AVFoundation 可解码格式

受 DRM、密码或平台账号保护的电子书与影音无法作为普通文件导入。老式 DOC 的复杂排版、文本框和嵌入对象可能无法完整转换；建议优先使用 DOCX。扫描版 PDF 可以按原页面查看，但没有可用于自动分章的文本。
